...
### Métodos importantes:
- `update_winrate()`: Este método calcula y actualiza el winrate del jugador basándose en el número de juegos jugados y ganados. Si el jugador no ha jugado juegos, su winrate será 0%.

```python
class Player(models.Model):
    user = models.OneToOneField(User, on_delete=models.CASCADE)
    team = models.ForeignKey(Team, on_delete=models.SET_NULL, null=True, blank=True)
    role = models.CharField(max_length=10, choices=ROLE_CHOICES, default=DEFAULT)  # Premium o Normal
